...
<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
	<header class="entry-header">
		<?php if ( is_single() ) : ?>
		<h1 class="entry-title"><?php the_title(); ?>  <?php edit_post_link( __( '[ Edit ]', 'luminescence' ), '<span class="edit-link">', '</span>' ); ?>
        </h1>
		<?php else : ?>
		<h1 class="entry-title">
			<a href="<?php the_permalink(); ?>" rel="bookmark"><?php the_title(); ?></a>  <?php edit_post_link( __( '[ Edit ]', 'luminescence' ), '<span class="edit-link">', '</span>' ); ?>
		</h1>
		<?php endif; // is_single() ?>
	</header><!-- .entry-header -->
